...
namespace zx81emulator.zx81 {
    ...
    export class ZX81 extends Machine implements ZX81ConfigDefs {
        ...
        public initialise(config : ZX81Config) {
            ...
            let i : number;
            let romlen : number;
            for(i = 0; i < 65536; i++) this.memory[i] = 7
            snap.memory_load("ROM/" + this.CurRom, 0, 65536, (() => { let f = function() { this.call = () => {
                this.ink = 0;
        ...
        public writebyte(Address : number, Data : number) {
            if(this.zx81opts.chrgen === ZX81ConfigDefs.CHRGENQS && Address >= 33792 && Address <= 34815) {
        ...
        public opcode_fetch(Address : number) : number {
            ...
            inv = (data & 128) !== 0;
            if(!bit6) {
                if((this.zx81opts.chrgen === ZX81ConfigDefs.CHRGENCHR16 && (this.z80.I & 1) !== 0) || (this.zx81opts.chrgen === ZX81ConfigDefs.CHRGENQS && this.zx81opts.enableqschrgen)) data = ((data & 128) >> 1) | (data & 63); else data = data & 63;
                if(this.z80.I < 64 || (this.z80.I >= 128 && this.z80.I < 192 && this.zx81opts.chrgen === ZX81ConfigDefs.CHRGENCHR16)) {
                    if((this.zx81opts.chrgen === ZX81ConfigDefs.CHRGENQS && this.zx81opts.enableqschrgen)) data = this.font[(data << 3) | this.rowcounter]; else data = this.readbyte(((this.z80.I & 254) << 8) + (data << 3) | this.rowcounter);
                } else data = 255;
                update = true;
            }
            if(update) {
        ...
        public readport(Address : number) : number {
            if((Address & 1) === 0) {
                ...
                keyb = (Address / 256|0);
                for(i = 0; i < 8; i++) {
                    if((keyb & (1 << i)) === 0) data |= KBStatus.ZXKeyboard_$LI$()[i];
                }
                return ((~data) & 255);
        ...
        public do_scanline(CurScanLine : Scanline) : number {
            ...
            do {
                ...
                switch((this.LastInstruction)) {
                    case ZX81.LASTINSTOUTFD:
                        ...
                        if(CurScanLine.sync_len !== 0) CurScanLine.sync_valid = ZX81ConfigDefs.SYNCTYPEV;
                ...
                if(this.hsync_counter <= 0) {
                    ...
                    if(this.HSYNC_generator && CurScanLine.sync_len === 0) {
                        ...
                        CurScanLine.sync_valid = ZX81ConfigDefs.SYNCTYPEH;
            ...
            if(CurScanLine.sync_valid === ZX81ConfigDefs.SYNCTYPEV) {
                this.hsync_counter = this.tperscanline;
            }
            return (tstotal);
        }
    ...
    ZX81["__class"] = "zx81emulator.zx81.ZX81";
    ZX81["__interfaces"] = ["zx81emulator.config.ZX81ConfigDefs"];
